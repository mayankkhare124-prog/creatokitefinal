          ...
          <div key={s.key} style={{ display:'flex', alignItems:'center', flex:1 }}>
            <div style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:3,flex:1 }}>
              <div style={{
                ...
                display:'flex',alignItems:'center',justifyContent:'center',
                background:done?'var(--acc2)':active?'var(--p)':'rgba(255,255,255,0.06)',
                border:active?'2px solid var(--p2)':'1px solid rgba(255,255,255,0.1)',
                color:done||active?'#fff':'var(--t3)',
                boxShadow:active?'0 0 10px rgba(108,99,255,0.5)':'none',
                transition:'all 0.3s',flexShrink:0,
              }}>{done?'✓':i+1}</div>
              <span style={{ fontSize:9,color:active?'var(--p2)':done?'var(--acc2)':'var(--t3)',textAlign:'center',lineHeight:1.2 }}>{s.label}</span>
            </div>
            {i<WF_STEPS.length-1&&<div style={{ height:1,width:12,background:done?'var(--acc2)':'rgba(255,255,255,0.08)',flexShrink:0,marginBottom:14,transition:'background 0.3s' }}/>}
          </div>
        );
      })}
    </div>
